...
include_once($_SERVER['DOCUMENT_ROOT']."/bitrix/components/black_mist/delivery.packages/getcurier.php");
$modes = array(
	'list',
	'upload',
	'upload_settings'
...
if ($arResult['MODE'] != 'close')
{
				...
				if ($obj)
				{
					...
					foreach ($obj as $m)
					{
						...
						foreach ($m['DeliveryNotes'] as $nakl)
						{
							...
							$courier = $nakl["CURIER"];
							/*if($nakl["CURIER"]!=""){
								$courNaklad["CURIER"] = $courier;
								$courNaklad["NAKLADNAYA"] = $numbernakl;
							}
							array_push($courierNakladnye, $courNaklad);*/
							...
							foreach ($nakl['DeliveryEvents'] as $ev)
							{
								...
								{ 
									...
									unset($numbers[$numbernakl]);
									unset($events[$numbernakl]);
									continue;
									//$count_deliverd++;
								}
	...
	$uk = $arResult['UK'];
	$ninn = $arResult['CURRENT_INN'];
	$currentip = GetSettingValue(683, false, $arResult['UK']);
	$currentlink = GetSettingValue(704, false, $arResult['UK']);
	$login1c = GetSettingValue(705, false, $arResult['UK']);
	$pass1c = GetSettingValue(706, false, $arResult['UK']);

	$tfrom = $arResult['LIST_FROM_DATE_FOR_1C'];

    $tTo = $arResult['LIST_TO_DATE_FOR_1C'];
	$curType = $arResult['CURRENT_TYPE_M'];

//print_r(getCourierNakl(35639831,$uk,$ninn,$currentip,$currentlink,$login1c,$pass1c,$tfrom,$tTo,$curType));
	if($arResult['CURRENT_AGENT']>0){
		$arFilter = Array(
		 "IBLOCK_ID"=>IntVal(97),
		 "ACTIVE"=>"Y", 
		 "=PROPERTY_AGENT"=>IntVal($arResult['CURRENT_AGENT'])
		 );
		$res = CIBlockElement::GetList(Array("SORT"=>"DESC"), $arFilter, Array("NAME","XML_ID"));
		while($ar_fields = $res->GetNext())
		{ 
			$curId = $ar_fields['XML_ID'];
			if(getCourierNakl($curId,$uk,$ninn,$currentip,$currentlink,$login1c,$pass1c,$tfrom,$tTo,$curType)){
				$ar_fields['NAKLADNYE'] = getCourierNakl($curId,$uk,$ninn,$currentip,$currentlink,$login1c,$pass1c,$tfrom,$tTo,$curType);
			}
		 	$arResult['CURRENT_COURIER'][] = $ar_fields;
		}
	}
