					...
					else
					{
						...
						else
						{
							...
							$client = new SoapClient(
								'http://msk.delivery-russia.ru/sd_msk/ws/DashboardExchange.1cws?wsdl',  
									array('login' => 'DMSUser', 'password' => "1597534682",'exceptions' => false)
							);	
							$result = $client->GetPrefixAgent($arParamsJson);
							$mResult = $result->return;
							$obj = json_decode($mResult, true);
							if (strlen($obj['Prefix_'.$arResult['AGENT']['PROPERTY_INN_VALUE']]))
							{
								$arResult['AGENT']['PREFIX_1C'] = iconv('utf-8', 'windows-1251', $obj['Prefix_'.$arResult['AGENT']['PROPERTY_INN_VALUE']]);
							}
							else
							{
								$arResult["OPEN"] = false;
								$arResult["ERRORS"][] = GetMessage("ERR_PREFIX_INN");
							}
						}
						*/
					}
		...
		*/
		...
		if ($mode == 'list')
		{
			...
			if ($arResult['ADMIN_AGENT'])
			{
				...
				{
					$arResult['LIST_OF_AGENTS'] = AvailableClients(false);
				}
...
AddMessage2Log($arResult['AGENT']['PROPERTY_INN_VALUE']);
					...
					if (count($arResult["ERR_FIELDS"]) == 0)
					{
						...
						*/
						...
						{		
							...
							{
								$logs = $_SERVER['DOCUMENT_ROOT'].'/logs/log-make.txt';
								$errors_file = fopen($logs,'a');
								fwrite($errors_file,$z_id.' '.$number."\n");
								fwrite($errors_file,"\n");
								fclose($errors_file);
		...
		if ($mode == '1c')
		{
			...
			{
				if (($_POST['login'] == 'DMSUser') && ($_POST['pass'] == '1597534682'))
				{
                    $arRes = array();
                    ...
                    else
                    {
                        if (intval($arRes['ID']) > 0)
                        {
                            ...
                            {
                                $arResult["ERRORS"][] = 'Заявка с таким ID не найдена';
                            }
                        }
                        else
                        {
