...
function GetCurrentAgent($u_id)
{
    ...
    if (intval($arUser['UF_COMPANY_RU_POST']) > 0)
    {
        ...
        $res = CIBlockElement::GetList(
            ...
            false,
            false,
            array("PROPERTY_TYPE","PROPERTY_DEMO","NAME","PROPERTY_ID_IN",
                "PROPERTY_LEGAL_NAME","PROPERTY_UK","PROPERTY_INN", "PROPERTY_CITY",
                'PROPERTY_CITY.NAME' ,'PROPERTY_ON_PAGE', 'PROPERTY_REGION',
                'PROPERTY_PREFIX_REPORTS','PROPERTY_IM_BY','PROPERTY_PHONES',
                'PROPERTY_ADRESS','PROPERTY_ADRESS_FACT')
        );
...
function GetCityId($city, $onlyone = false)
{
    ...
    if (isset($from_arr[1]))
    {
        $res_0 = CIBlockSection::GetList(["SORT"=>"ASC"], ["NAME"=>trim($from_arr[1]),"IBLOCK_ID"=>6],false);
...
function CheckCityToHave($id_city, $price_id, $w = 0, $s1 = 0,$s2 = 0, $s3 = 0, $double = 1, $price_zabor = 0, $iblock = 51, $type_delivery = array("Стандарт"), $kobw = 0, $returnOne = true)
{
    ...
    $res = CIBlockElement::GetList(
        ["ID"=>"ASC"],
        ["IBLOCK_ID" => $iblock, "ACTIVE" => "Y", "ID" => $price_id],
        false,
        ["nTopCount" => 1],
        ["ID","PROPERTY_CITIES", "PROPERTY_FILE"]
    );
    if($ob = $res->GetNextElement())
    {
        $arFields = $ob->GetFields();
    ...
    if (count($arFields) > 0)
    {
        ...
        $db_props = CIBlockElement::GetProperty($iblock, $price_id_bd, ["sort" => "asc"], ["CODE"=>"FILE"]);
        if($ar_props = $db_props->Fetch())
        {
...
function GetRoleOfUser($user)
{
    ...
    $arUser = $rsUser->Fetch();
    if ((int)$arUser["UF_ROLE"] > 0)
    {
        $res = CIBlockElement::GetByID((int)$arUser["UF_ROLE"]);
        if ($ar_res = $res->GetNext())
        {
...
function GetListContractors($creator, $type = 777, $nav = true, $name_nav = '', $sort = ["NAME"=>"ASC"], $name = false, $admin_agent = false, $all = false, $branch = false, $on_page_stat = 0)
{
    $json_data = [];
    if ($nav)
    {
        if ((int)$on_page_stat > 0)
        {
            $ON_PAGE = (int)$on_page_stat;
        }
        else
        {
            if ((int)$_GET['on_page'] > 0)
            {
            ...
            {
                if ((int)$_SESSION['ON_PAGE_COMPS'] > 0)
                {
        ...
        $arNav = ["nPageSize" => $ON_PAGE];
    }
    else
    {
        $arNav = false;
    }
    $filter = ["IBLOCK_ID"=>84, "PROPERTY_CREATOR"=> $creator];
    if (((int)$type > 0) && ((int)$type != 777))
    {
        $filter['PROPERTY_TYPE'] = (int)$type;
    }
    if (!$all)
    {
        $filter['ACTIVE'] = "Y";
    ...
    $res = CIBlockElement::GetList(
        ...
        false,
        $arNav,
        ["ID","NAME","ACTIVE","PROPERTY_NAME", "PROPERTY_INN",  "PROPERTY_PHONE",
            "PROPERTY_CITY", "PROPERTY_INDEX", "PROPERTY_ADRESS", "PROPERTY_CITY.NAME", "PROPERTY_CREATOR.NAME"]
    );
    if ($nav)
    {
    ...
    while($ob = $res->GetNextElement()) {
        ...
        $coordArray['inn'] = $arFields['PROPERTY_INN_VALUE'];
...
{
    $a = [];
    $filter = ["IBLOCK_ID" => 40, "PROPERTY_INN" => $inn];
    if ($type)
    {
...
function makeManifestOrderfromDMSOrder($arOrder = false , $idorder = 0, $number = '')
{
    ...
    $arFilter = array(
        'ID', 'DATE_CREATE', 'PROPERTY_N_ZAKAZ_IN', 'PROPERTY_STATE',
        'PROPERTY_N_ZAKAZ', 'PROPERTY_CREATOR', 'PROPERTY_CITY', 'PROPERTY_CITY.NAME',
        'PROPERTY_PREFERRED_TIME', 'PROPERTY_URGENCY_ORDER', 'PROPERTY_DELIVERY_LEGAL',
        'PROPERTY_WHEN_TO_DELIVER', 'PROPERTY_CONDITIONS', 'PROPERTY_TIME_PERIOD',
        'PROPERTY_RECIPIENT', 'PROPERTY_PHONE', 'PROPERTY_ADRESS', 'PROPERTY_COST_2',
        'PROPERTY_PLACES', 'PROPERTY_SIZE_1', 'PROPERTY_SIZE_2', 'PROPERTY_SIZE_3',
        'PROPERTY_WEIGHT','PROPERTY_PACK_GOODS'
    );
    $arErrors = array();
    if (is_array($arOrder))
    {
    ...
    if (is_array($reqv2))
    {
        if (($reqv2['PROPERTY_STATE_ENUM_ID'] == 54) || ($reqv2['PROPERTY_STATE_ENUM_ID'] == 118))
        {
            ...
            if ((int)$reqv2['PROPERTY_URGENCY_ORDER_VALUE'] == 172)
            {
                $comment = strlen($comment) ? 'Срочный заказ! '.$comment : 'Срочный заказ!';
            }
            if ((int)$reqv2['PROPERTY_DELIVERY_LEGAL_VALUE'] == 1)
            {
            ...
            foreach ($moths as $k => $m)
            {
                ...
                {
                    $d = str_pad((int)substr($reqv2['PROPERTY_WHEN_TO_DELIVER_VALUE'], 0, 2),2,'0',STR_PAD_LEFT);
            ...
            if ($reqv2['PROPERTY_CONDITIONS_ENUM_ID'] == 37)
            {
                ...
                switch ($reqv2['PROPERTY_TIME_PERIOD_ENUM_ID'])
                {
                    case 215:
                        ...
                        $TIME_DELIVERY = " Время доставки с 10:00 до 14:00";
                        break;
                    case 216:
                        $DATE_TAKE_FROM .= ' 15:00:00';
                        $DATE_TAKE_TO .= ' 18:00:00';
                        $TIME_DELIVERY = " Время доставки с 15:00 до 18:00";
                        break;
                    default:
                        $DATE_TAKE_FROM .= ' 10:00:00';
                        $DATE_TAKE_TO .= ' 18:00:00';
                        $TIME_DELIVERY = " Время доставки с 10:00 до 18:00";
                }
            }
            $comment .= $TIME_DELIVERY;
            $reqv2['PACK_GOODS'] = '';
...
/* выгрузка в excel массива */

function get_excel_file($data, $title, $columns, $postfix=""){
    set_include_path($_SERVER['DOCUMENT_ROOT'].'/bitrix/_black_mist/PhpExcel/Classes/');
    include_once 'PHPExcel.php';
...
// Вставляем заголовок в "B2"

    //$sheet->setCellValueByColumnAndRow($columnPosition, $startLine, $title);
...
// Выравниваем по центру
    ...
    {
        if($m==0){
            $sheet->getColumnDimension($arJ[$m])->setWidth(50);
        }else{
            $sheet->getColumnDimension($arJ[$m])->setWidth(40);
        }
    }
    // $sheet->getStyle('A0:'.$endLetter.'3')->applyFromArray($head_style);
...
// Формируем список
    foreach ($data as $key=>$catItem) {
        ...
        // Вставляем информацию об имени и цвете
    ...
    $file_name = "Data_list-{$postfix}.xls";
...
function getExtensionPath($pathname) {
    return substr($pathname, strrpos($pathname, '.') + 1);
}


/* функции перегоняют xml файл в массив

   пример использования
$tree = xml_get_tree('country_world.xml');
...
/* конец функции перегоняют xml файл в массив */
 ...
 * @return mixed
 */
function GetInfoArr($code = '', $id = '', $iblock_id = 0, $arSelect = [], $arFilter = [], $one=true, $prop=true){
    if(empty($arSelect)){
        $arSelect = [
            "ID", "IBLOCK_ID", "NAME", "DATE_CREATE", "PROPERTY_*"
        ];
    }
    if($code){
        $arFilter = [
...
function soap_include($id_uk){

    $currentip = GetSettingValue(683, false, $id_uk);
    $currentport = GetSettingValue(761, false, $id_uk);
    $currentlink = GetSettingValue(704, false, $id_uk);
    $login1c = GetSettingValue(705, false, $id_uk);
    $pass1c = GetSettingValue(706, false, $id_uk);
    /*AddToLogs('1c_pickup', [
        'functions-9127'=>[
            'currentip'=>$currentip,
...
function UserData($id){
    $by='id';
    $order="desc";
    $rsUsers = CUser::GetList($by, $order, ['ID'=>$id], []);
 ...
 * @param int $client
 * @param int $inn_client
 * @return bool|float|mixed
 */
function getSumDev($total_weight, $total_gabweight, $city_recipient, $client = 0, $inn_client = 0){
    $username =  iconv('windows-1251','utf-8',"DMSUser");
    $password =  iconv('windows-1251','utf-8',"1597534682");
    $host_api = '';
    if($inn_client){
        $host_api = "http://92.42.209.242/sd_msk/hs/Delivery/Account/7717739535/{$inn_client}/GetTarif?ves=$total_weight&vesv=$total_gabweight&idcity2=$city_recipient&deliverytype=c";
    }
    if($client == 52254529){
 ...
 * @return bool|float|mixed
 */
function getSumDelivered($total_weight, $total_gabweight, $city_sender, $city_recipient, $client = 0, $inn_client = 0, $type_dev= 'с'){
    $username =  iconv('windows-1251','utf-8',"DMSUser");
    $password =  iconv('windows-1251','utf-8',"1597534682");
    $host_api = '';
    if($inn_client){
        $host_api = "http://92.42.209.242/sd_msk/hs/Delivery/Account/7717739535/{$inn_client}/GetTarif?ves=$total_weight&vesv=$total_gabweight&idcity1=$city_sender&idcity2=$city_recipient&deliverytype=$type_dev";
    }
    if($client == 52254529){
        $host_api = "http://92.42.209.242/sd_msk/hs/Delivery/Account/7717739535/7701902970/GetTarif?ves=$total_weight&vesv=$total_gabweight&idcity1=$city_sender&idcity2=$city_recipient&deliverytype=$type_dev";
    }
    if($client == 56103010){
        $host_api = "http://92.42.209.242/sd_msk/hs/Delivery/Account/7717739535/7728178835/GetTarif?ves=$total_weight&vesv=$total_gabweight&idcity1=$city_sender&idcity2=$city_recipient&deliverytype=$type_dev";
    }
    // AddToLogs( 'getNumber',['api'=>$host_api]);
    if(!$host_api){
        return false;
    }
    $url = iconv('windows-1251','utf-8',$host_api);
    if ($ch = curl_init()){
        ...
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        $output = curl_exec($ch);
        // AddToLogs( 'getNumber',['api'=>$host_api, '$output'=>$output]);
        curl_close($ch);
        $sum_dev =  (float)iconv('utf-8','windows-1251',$output);
        return $sum_dev ;
    }
    return false;
}
...
function setAppForAgent($result, $id_uk, $arrC, $number_uid, $creator, $inn_agent=false, $id_rec=false){
    if ($id_rec){
        $arFilter = ["IBLOCK_ID" => 117, "ACTIVE" => "Y",  "ID" => $id_rec];
        $arSelect = [
            "ID","NAME", "ACTIVE", "IBLOCK_ID", 'PROPERTY_1061'
        ];
        ...
        $action = 'update';
        AddToLogs('1c_pickup_upd', ['functions-9319'=> $action] );
    }else{
        $action = 'add';
    }
    if(!empty($result['Doc'])){
        ...
        foreach($result['Doc'] as $key=>$val){
            ...
            if($key === 'СпециальныеИнструкции')  {$arResult['APP_FOR_AGENT']['Instruction'] = $value; continue;}   //   [СпециальныеИнструкции] => Заранее позвонить отправителю Забрать 18.02.2021 с 10:00 до 16:00
        ...
        $arResult['APP_FOR_AGENT']['DataAppCreate'] = $data_create;
        if($action === 'add')
            $arResult['APP_FOR_AGENT']['DataAppCreate'] = date('d.m.Y H:i:s');
        ...
        $arResult['APP_FOR_AGENT']['EVENT_LAST_DATE_F'] = date('d.m.Y H:i:s', strtotime($arResult['APP_FOR_AGENT']['EVENT_LAST_DATE']));
        ...
        $arLoadArray = [
            ...
            "PROPERTY_VALUES" => [
                ...
                1080 => $arResult['APP_FOR_AGENT']['EVENT_LAST_DATE_F'],
                ...
                1072 => $arResult['APP_FOR_AGENT']['SpecU'],
                1073 => $arResult['APP_FOR_AGENT']['Desc_items'],
                1074 => $arResult['APP_FOR_AGENT']['DocP'],
                1075 => $arResult['APP_FOR_AGENT']['Id_uk'],
                1076 => $arResult['APP_FOR_AGENT']['INN_Agent'],
        ...
        if($action==='add'){
            ...
            if(!$rec_id){
                AddToLogs('1c_pickup', ['functions-9497' => ['Error' =>
                    "Ошибка добавления заявки. UID - {$arResult['APP_FOR_AGENT']['UID']}"], '$arLoadArray' => $arLoadArray] );
                exit();
            }
            AddToLogs('1c_pickup', ['functions-9501' => ['SUCCESS' => $rec_id] ]);
            exit();
        }
        ...
        if($action==='update'){
            $rec_id_upd = $uid['ID'];
            $el->Update($rec_id_upd, ['NAME'=>$arResult['APP_FOR_AGENT']['NumberApp']]);
            $props = [];
            $props[1023] = $arResult['APP_FOR_AGENT']['NumberInvoice'];
            $props[1059] = $arResult['APP_FOR_AGENT']['EVENTS'];
            $props[1024] = $arResult['APP_FOR_AGENT']['NumberApp'];
            $props[1025] = $arResult['APP_FOR_AGENT']['Sender'];
            $props[1026] = $arResult['APP_FOR_AGENT']['SenderName'];
            $props[1027] = $arResult['APP_FOR_AGENT']['SenderCompany'];
            $props[1028] = $arResult['APP_FOR_AGENT']['SenderPhone'];
            $props[1029] = $arResult['APP_FOR_AGENT']['SenderIndex'];
            $props[1030] = $arResult['APP_FOR_AGENT']['SenderCountry'];
            $props[1031] = $arResult['APP_FOR_AGENT']['SenderRegion'];
            $props[1032] = $arResult['APP_FOR_AGENT']['SenderCity'];
            $props[1033] = $arResult['APP_FOR_AGENT']['SenderAdress'];
            $props[1034] = $arResult['APP_FOR_AGENT']['SenderGeoPlace'];
            $props[1035] = $arResult['APP_FOR_AGENT']['SenderPrim'];
            $props[1036] = $arResult['APP_FOR_AGENT']['Recipient'];
            $props[1037] = $arResult['APP_FOR_AGENT']['RecipientName'];
            $props[1038] = $arResult['APP_FOR_AGENT']['RecipientCompany'];
            $props[1039] = $arResult['APP_FOR_AGENT']['RecipientPhone'];
            $props[1040] = $arResult['APP_FOR_AGENT']['RecipientIndex'];
            $props[1041] = $arResult['APP_FOR_AGENT']['RecipientCountry'];
            $props[1042] = $arResult['APP_FOR_AGENT']['RecipientRegion'];
            $props[1043] = $arResult['APP_FOR_AGENT']['RecipientCity'];
            $props[1060] = $arResult['APP_FOR_AGENT']['RecipientAdress'];
            $props[1044] = $arResult['APP_FOR_AGENT']['RecipientGeoPlace'];
            $props[1045] = $arResult['APP_FOR_AGENT']['RecipientPrim'];
            $props[1046] = $arResult['APP_FOR_AGENT']['RecipientDataApp'];
            $props[1047] = $arResult['APP_FOR_AGENT']['TimeFrom'];
            $props[1048] = $arResult['APP_FOR_AGENT']['TimeTo'];
            $props[1049] = $arResult['APP_FOR_AGENT']['TypePayer'];
            $props[1050] = $arResult['APP_FOR_AGENT']['SummPay'];
            $props[1051] = $arResult['APP_FOR_AGENT']['CitySenderId'];
            $props[1052] = $arResult['APP_FOR_AGENT']['CityRecipientId'];
            $props[1053] = $arResult['APP_FOR_AGENT']['RecipientDataAppFormat'];
            $props[1054] = $arResult['APP_FOR_AGENT']['Creator'];
            $props[1055] = $arResult['APP_FOR_AGENT']['TypePay'];
            $props[1056] = $arResult['APP_FOR_AGENT']['UID'];
            $props[1057] = $arResult['APP_FOR_AGENT']['CreatorName'];
            $props[1061] = $arResult['APP_FOR_AGENT']['DataAppCreate'];
            $props[1062] = $arResult['APP_FOR_AGENT']['EVENT_LAST'];
            $props[1063] = $arResult['APP_FOR_AGENT']['EVENT_LAST_DATE'];
            $props[1080] = $arResult['APP_FOR_AGENT']['EVENT_LAST_DATE_F'];
            $props[1064] = $arResult['APP_FOR_AGENT']['EVENT_LAST_INFO'];
            $props[1059] = $arResult['APP_FOR_AGENT']['EVENTS'];
            $props[1066] = $arResult['APP_FOR_AGENT']['TypePayText'];
            $props[1065] = $arResult['APP_FOR_AGENT']['TypePayerText'];
            $props[1067] = $arResult['APP_FOR_AGENT']['Instruction'];
            $props[1068] = $arResult['APP_FOR_AGENT']['weight'];
            $props[1069] = $arResult['APP_FOR_AGENT']['weightV'];
            $props[1070] = $arResult['APP_FOR_AGENT']['places'];
            $props[1071] = $arResult['APP_FOR_AGENT']['info'];
            $props[1072] = $arResult['APP_FOR_AGENT']['SpecU'];
            $props[1073] = $arResult['APP_FOR_AGENT']['Desc_items'];
            $props[1074] = $arResult['APP_FOR_AGENT']['DocP'];
            $props[1075] = $arResult['APP_FOR_AGENT']['Id_uk'];
            $props[1076] = $arResult['APP_FOR_AGENT']['INN_Agent'];
            CIBlockElement::SetPropertyValuesEx($rec_id_upd, 117,$props);

           // AddToLogs('1c_pickup_upd', [ '$rec_id_upd-function-9508' => $rec_id_upd, '$arLoadArray-function-9567' => $props]);
        }
        ...
        if ($creator)
        {
            ...
            {
                if($action==='update'){
                    $arMessageFields['NOTIFY_MESSAGE'] = "Обновлена Заявка {$arResult['APP_FOR_AGENT']['NumberApp']} для агента  {$arResult['APP_FOR_AGENT']['CreatorName']} ";
                }
                if($action==='add'){
                    $arMessageFields['NOTIFY_MESSAGE'] = "Поступила Заявка {$arResult['APP_FOR_AGENT']['NumberApp']} для агента  {$arResult['APP_FOR_AGENT']['CreatorName']} ";
                }
        ...
        if($action==='add'){
            // AddToLogs('1c_pickup', ['functions-add-9601'=> $arResult['APP_FOR_AGENT'],
            //     'rec_id' => $rec_id, 'user_from'=>$users_to]);
            return $rec_id;
        }

        if($action==='update'){
            //AddToLogs('1c_pickup_upd', ['functions-update-9607'=> $arResult['APP_FOR_AGENT'],
