...
/*************************************************************************************************/
	...
	if (strlen($arResult["ERROR"]) <= 0)
	{
		...
		if (($_POST['WEB_FORM_ID'] == $arParams['WEB_FORM_ID'] || $_POST['WEB_FORM_ID'] == $arResult['arForm']['SID']) && (strlen($_REQUEST["web_form_submit"])>0 || strlen($_REQUEST["web_form_apply"])>0))
		{
			...
			if (strlen(trim($_POST['g-recaptcha-response'])) <= 0)
			{
				if ($arParams['USE_EXTENDED_ERRORS'] == 'Y')
				{
					$arResult["FORM_ERRORS"][] = GetMessage("REGISTER_WRONG_CAPTCHA");
				}
				else
				{
					$arResult["FORM_ERRORS"] .= '<br>'.GetMessage("REGISTER_WRONG_CAPTCHA");
				}
			}
		...
		$arResult = array_merge(
			...
			array(
				...
				"FORM_HEADER" => sprintf( // form header (<form> tag and hidden inputs)
					"<form name=\"%s\" action=\"%s\" method=\"%s\" enctype=\"multipart/form-data\" class=\"form-horizontal\">",
