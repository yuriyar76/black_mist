...
require_once __DIR__ . '/AllFunc.php';
require_once __DIR__ . '/CalcI.php';
/**
 * Class Calc
 */
class Calc extends AllFunc implements CalcI
{
    ...
    public function __construct($in_post, $in_get, $inner = false)
    {
        ...
        if($inner){
            ...
            $this->arrPost = AllFunc::arFromUtfToWin($in_post);
        }
            ...
            //AllFunc::AddToLogs("testCalс", $this->arrPost);
            $city_0_ok = explode(',', $this->arrPost['city_0']);
            if(count($city_0_ok) === 3){
                if (empty($this->arrPost['citycode_0'])){
                    $this->arrPost['citycode_0'] = AllFunc::GetCityId($this->arrPost['city_0']);
                }
                $city_0_ok = true;
            }else{
                $city_0_ok = false;
            ...
            if(count($city_1_ok) === 3){
                if (empty($this->arrPost['citycode_1'])){
                    $this->arrPost['citycode_1'] = AllFunc::GetCityId($this->arrPost['city_1']);
                }
                $city_1_ok = true;
            } else{
                $city_1_ok = false;
    ...
    public function GetResult()
    {
        ...
        $arr_s = AllFunc::GetInfoArr(false, $this->id_sender, 6, $arSelect);
        $arr_r = AllFunc::GetInfoArr( false, $this->id_recipient, 6, $arSelect);
        if(is_array($arr_s))
        {
    ...
    public function routCalc()
    {
      if($this->option_1) {
            if ($this->Res['SENDER']['IBLOCK_SECTION_ID'] == $this->Res['RECIPIENT']['IBLOCK_SECTION_ID']) {
                ...
                if ($q) {
                    $this->Res['TARIF_ITOG'] = $this->GetTarifSPB($this->Res, Calc::TMain, Calc::THMain);
                    $this->Res['TIMEDEV'] = Calc::DEFAULT_DEV_CITY;
                } else {
        ...
        elseif($this->option_3){
            ...
            */
            ...
            // AllFunc::AddToLogs("testCalс", ["MESS"=>" Москва - Москва ",  $Res['TARIF_ITOG_MSK']]);
        }
        elseif($this->option_4){
            $this->flag = true;
            $this->Res['TARIF_ITOG'] = 0;
            if($this->Res['RECIPIENT']['IBLOCK_SECTION_ID'] == 641){
                $dist_arr = AllFunc::GetInfoArr( false,$this->Res['RECIPIENT']['ID'], 6, $this->arSelect);
            }elseif($this->Res['SENDER']['IBLOCK_SECTION_ID'] == 641){
                $dist_arr = AllFunc::GetInfoArr( false,$this->Res['SENDER']['ID'], 6, $this->arSelect);
            }
            ...
            $this->Res['TIMEDEV'] = Calc::DEFAULT_DEV_C;  /* переделать как дадут сроки */
            // AllFunc::AddToLogs("testCalс", ["MESS"=>"  Московская область - СПБ ", $dist_arr]);
        }
        elseif($this->option_5){
            $this->flag = true;
            $dist_arr_sender =  AllFunc::GetInfoArr( false,$this->Res['SENDER']['ID'], 6, $this->arSelect);
            $dist_arr_recipient =  AllFunc::GetInfoArr( false,$this->Res['RECIPIENT']['ID'], 6, $this->arSelect);
            ...
            if(($dist_mkad <= DEFAULT_DISTANCE && $dist_kad <= DEFAULT_DISTANCE)){
                $tr =  AllFunc::GetInfoArr( false,52440321, 111, $this->arSelect);
            }
            if(($dist_mkad <= DEFAULT_DISTANCE && $dist_kad > DEFAULT_DISTANCE)){
                $tr =  AllFunc::GetInfoArr( false,52440573, 111, $this->arSelect);
            }
            if(($dist_mkad > DEFAULT_DISTANCE && $dist_kad <= DEFAULT_DISTANCE)){
                $tr =  AllFunc::GetInfoArr( false,52440575, 111, $this->arSelect);
            }
            if(($dist_mkad > DEFAULT_DISTANCE && $dist_kad > DEFAULT_DISTANCE)){
                $tr =  AllFunc::GetInfoArr( false,52440576, 111, $this->arSelect);
            }
            ...
            $this->Res['TIMEDEV'] = Calc::DEFAULT_DEV_MSO;        /* 4-6 переделать как дадут сроки */
            if($ext == "Y"){
                $this->GetTarifMSKSpbOBL($this->Res, false);
            }
            // AllFunc::AddToLogs("testCalс", ["MESS"=>"  Московская область - Ленинградская область ", $Res]);
        }
        ...
        elseif($this->option_9){
            ...
            if(($this->Res['SENDER']['IBLOCK_SECTION_ID'] != 656 &&
                ...
                AllFunc::AddToLogs("testCalс", $this->tarif);
        ...
        elseif($this->option_12){
            ...
            AllFunc::AddToLogs("testCalс", ["MESS"=>" Питер  - ЛО область ", $this->Res]);
        }
        ...
        elseif($this->option_14){
            ...
            if($req==="Y"){
                $this->coeff = Calc::C_1;
                $this->Res['ZONEDEV'] = Calc::DEFAULT_ZONE_REGION;
                $this->Res['TIMEDEV'] = Calc::DEFAULT_DEV_REGION;
                // AllFunc::AddToLogs("testCalс", ["MESS"=>" внутри областей кроме подмосковья и лен области ", $Res]);
            }
            elseif($req==="C"){
                $coeff = $this->GetRegion($Res);
                //AllFunc::AddToLogs("testCalс", ["MESS"=>" внутри областей, основные города ", $Res]);
            }
            else{
        ...
        {
            $this->Res['ERROR']['ERR_RASCHET'] = \Bitrix\Main\Localization\Loc::GetMessage('ERR_RASCHET');
            AllFunc::AddToLogs("testCalс", ["MESS"=>" Общая ошибка, по введенным данным нас. пунктов расчет не возможен ", $Res]);
        }
        $this->Res['RASCH_COEFF'] = $this->coeff;
        if(!$this->flag)
        {
    ...
    public function GetRegion(array &$Res)
    {
        ...
        if($zSend > 0 && $zRep > 0){
            ...
            $zone = AllFunc::GetInfoArr( $code_recipient_zone,false, 101, $this->arSelect);
            ...
            if((int)$zoneDelivery>0){
                //dump($zoneDelivery);
                $time = AllFunc::GetInfoArr( false,49531756, 102, $this->arSelect);
    ...
    function GetMskObl(array &$Res, $flag = false){
        ...
        $dist_mkad_sender_arr = AllFunc::GetInfoArr( false,$Res['SENDER']['ID'], 6, $this->arSelect);
        $dist_mkad_recipient_arr =  AllFunc::GetInfoArr( false,$Res['RECIPIENT']['ID'], 6,  $this->arSelect);
        ...
        // AllFunc::AddToLogs("testCalc",  [$dist_mkad_sender]);
        // AllFunc::AddToLogs("testCalc",  [$dist_mkad_recipient]);
        ...
        if($dist_mkad_sender>0 && $dist_mkad_recipient>0 && !$flag){
            ...
            if($dist_mkad_sender <= self::DEFAULT_DISTANCE && $dist_mkad_recipient <= self::DEFAULT_DISTANCE){
                $Res['ZONEDEV'] = \Bitrix\Main\Localization\Loc::GetMessage('MOS_OBL');
                $tr = AllFunc::GetInfoArr( false,51213823, 103,  $this->arSelect);
            ...
            if(($dist_mkad_sender <= self::DEFAULT_DISTANCE && $dist_mkad_recipient > self::DEFAULT_DISTANCE)||
                ...
                $tr = AllFunc::GetInfoArr( false,51213826, 103,  $this->arSelect);
                //echo "Тариф до 0,5кг $tarif_05 руб. <br>";
                // echo "Тариф от 0,5кг-1,0кг $tarif_1 руб. <br>";
                //echo "Плюс на каждый кг свыше 1кг $tarif_hight руб. <br>";
            }
            ...
            if($dist_mkad_sender > self::DEFAULT_DISTANCE && $dist_mkad_recipient > self::DEFAULT_DISTANCE){
                $tr = AllFunc::GetInfoArr( false,51213827, 103,  $this->arSelect);
                //echo "Тариф до 0,5кг $tarif_05 руб. <br>";
                //echo "Тариф от 0,5кг-1,0кг $tarif_1 руб. <br>";
                //echo "Плюс на каждый кг свыше 1кг $tarif_hight руб. <br>";
            }
        ...
        elseif((($Res['SENDER']['ID'] == 8054) && $dist_mkad_recipient>0)||
            ...
            if(($dist_mkad_recipient > self::DEFAULT_DISTANCE && $Res['SENDER']['ID'] == 8054)||
                ...
                $tr = AllFunc::GetInfoArr( false,51213822, 103,  $this->arSelect);
            }
            /* Москва - область менее или равно 50 км  id - 51213813 */
            if(($dist_mkad_recipient <= self::DEFAULT_DISTANCE && $Res['SENDER']['ID'] == 8054)||
                ($dist_mkad_sender <= self::DEFAULT_DISTANCE && $Res['RECIPIENT']['ID'] == 8054)){
                $tr = AllFunc::GetInfoArr( false,51213813, 103,  $this->arSelect);
            }
        }
        elseif($flag){
            $tr = [];
    ...
    function GetLenOblNew(array $Res){
        ...
        $tr = AllFunc::GetInfoArr( false, $id_lo, 6, $this->arSelect);

        $req = AllFunc::GetInfoArr( false,
        ...
        //  AllFunc::AddToLogs("testCalс", ["MESS"=>$tr['PROPERTIES']['OUT_MKAD']]);
    ...
    function GetMskOblNew(array $Res) {
        ...
        $tr = AllFunc::GetInfoArr( false, $id_mo, 6,  $this->arSelect);
        if($tr['PROPERTIES']['SMALL_DEV']['VALUE']=='Y'){
            $tarif = 0;
            return $tarif;
        }else{
            $req = AllFunc::GetInfoArr( false, 52154989, 110,  $this->arSelect);
    ...
    function GetTarifSPBSpbOBL(&$Res, $f=true ){
        ...
        // AllFunc::AddToLogs("testCalс", ["MESS"=>" Питер - область ", "ID obl"=>$id_obl]);
        $arrDist = AllFunc::GetInfoArr(false, $id_obl, 6,  $this->arSelect);
        $distance = 0;
        ...
        if($distance <= self::DEFAULT_DISTANCE){
            $tr = AllFunc::GetInfoArr( false,52650410, 112,  $this->arSelect);
        }elseif($distance > self::DEFAULT_DISTANCE){
            $tr = AllFunc::GetInfoArr( false,52650411, 112,  $this->arSelect);
        }
        $Res['TIMEDEV']  = $tr['PROPERTIES']['TIME_DELIVERY']['VALUE'];
    ...
    function GetTarifMSKSpbOBL(&$Res, $f=true ){
        ...
        if($distance <= self::DEFAULT_DISTANCE){
            $tr = AllFunc::GetInfoArr( false,51897804, 107,  $this->arSelect);
        }elseif($distance > self::DEFAULT_DISTANCE){
            $tr = AllFunc::GetInfoArr( false,51897805, 107,  $this->arSelect);
        }
        $Res['TIMEDEV']  = $tr['PROPERTIES']['TIME_DELIVERY']['VALUE'];
        if($weight<=1.00 && $weight>0.00){
            $tarif = $tr['PROPERTIES']['TARIF_1']['VALUE'];
    ...
    function GetTarifNoMskObl(&$Res ){
        if( $Res['FULLWEIGTH'] <= 0.50 &&  $Res['FULLWEIGTH'] > 0){
            $tarif =  AllFunc::GetInfoArr( false,49528186, 100,  $this->arSelect);
        }elseif( $Res['FULLWEIGTH'] >0.5 &&  $Res['FULLWEIGTH'] <= 1.00){
            $tarif =  AllFunc::GetInfoArr( false,49528187, 100,  $this->arSelect);
        }
        elseif( $Res['FULLWEIGTH'] > 1.00 &&  $Res['FULLWEIGTH'] <= 2.00){
            $tarif =  AllFunc::GetInfoArr( false,49528190, 100,  $this->arSelect);
        }
        elseif( $Res['FULLWEIGTH'] > 2.00 &&  $Res['FULLWEIGTH'] <= 3.00){
            $tarif =  AllFunc::GetInfoArr( false,49528223, 100,  $this->arSelect);
        }
        elseif( $Res['FULLWEIGTH'] > 3.00 &&  $Res['FULLWEIGTH'] <= 4.00){
            $tarif =  AllFunc::GetInfoArr( false,49528230, 100,  $this->arSelect);
        }
        elseif( $Res['FULLWEIGTH'] > 4.00){

            $uplimit = (float) $Res['FULLWEIGTH'] - 4.00;
            $uplimit = ceil($uplimit);
            $tarif4 =  AllFunc::GetInfoArr( false,49528230, 100,  $this->arSelect);
            // dump($tarif4);
            $tarif4 = $tarif4['PROPERTIES']['ZONE_'.$Res['ZONEDEV']]['VALUE'];
            $tarif =  AllFunc::GetInfoArr( false,49528419, 100,  $this->arSelect);
            // dump($tarif);
        }
        $Res['TARIF'] =  $tarif;
    ...
    function GetTarifMskOblSPB(&$Res, $dist_mkad, $ext=false){
        ...
        if(!$ext){
            if(($dist_mkad > self::DEFAULT_DISTANCE)){
                $tr = AllFunc::GetInfoArr( false,52439704, 111,  $this->arSelect);
            }
            /* МО менее 50 км  id - 52439702 */
            if($dist_mkad < self::DEFAULT_DISTANCE){
                $tr = AllFunc::GetInfoArr( false,52439702, 111,  $this->arSelect);
            }
            $base_t = (float)$tr['PROPERTIES']['BASE_T']['VALUE'];
            $base_n = (float)$tr['PROPERTIES']['BASE_N']['VALUE'];
            $base_up = (float)$tr['PROPERTIES']['BASE_UP']['VALUE'];
    ...
    function GetMainCity($Res, $flag = false){
        if(!$flag){
            $req = AllFunc::GetInfoArr( false,$Res['SENDER']['ID'], 6,  $this->arSelect);
            ...
            $req_send = AllFunc::GetInfoArr( false,$Res['SENDER']['ID'], 6,  $this->arSelect);
            $req_rec = AllFunc::GetInfoArr( false,$Res['RECIPIENT']['ID'], 6,  $this->arSelect);
    ...
    function GetTarifMskPt(&$Res){
        $tr = AllFunc::GetInfoArr( false, 51897803, 107,  $this->arSelect);
        $Res['TIMEDEV']  = $tr['PROPERTIES']['TIME_DELIVERY']['VALUE'];
        ...
        // AllFunc::AddToLogs("testCalс", $Res);
        return $tr;
    }
    ...
    function GetTarifMsk(&$Res){
        $weight = (float)$Res['FULLWEIGTH'];
        $Res['STANDART'] = AllFunc::GetInfoArr(false, 51964190, 108,  $this->arSelect);
        $Res['EXPRESS_8'] = AllFunc::GetInfoArr(false, 51964682, 109,  $this->arSelect);
        $Res['EXPRESS_4'] = AllFunc::GetInfoArr(false, 51964677, 109,  $this->arSelect);
        $Res['EXPRESS_2'] = AllFunc::GetInfoArr(false, 51964676, 109,  $this->arSelect);
        return true;
    }
