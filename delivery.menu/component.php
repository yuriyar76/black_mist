...
$role_value = 0;
$current_u = $USER->GetID();
$rsUser = CUser::GetList(($by="last_name"), ($order="asc"),array("ID" => $current_u), array("SELECT" => array("UF_ROLE","UF_COMPANY_RU_POST")));
if($arUser = $rsUser->Fetch()) {
	$role_value = intval($arUser["UF_ROLE"]);
	$company = intval($arUser["UF_COMPANY_RU_POST"]);
}
if ($role_value > 0) {
	include_once($_SERVER['DOCUMENT_ROOT']."/bitrix/components/black_mist/delivery.packages/functions.php");
	$agent_array = GetCurrentAgent($current_u);
	$role_user = GetListRoles($role_value);
	$role_user = $role_user[$role_value];
	
	if ($agent_array['type'] == 51) $new_type = 159;
	if ($agent_array['type'] == 52) $new_type = 161;
	if ($agent_array['type'] == 53) $new_type = 160;
	
	if (array_key_exists($new_type,$role_user['FOR'])) {
		$only_dirs = $role_user['ROLES'][$new_type];
		$to_unset = array();
		foreach ($arResult as $k => $v) {
			if (strlen($v["LINK"]) > 1) {
				if ($v["LINK"][0] == '/') {
					$link = substr($v["LINK"],1);
				}
				$link = strstr($link, '/', true);
				if (!in_array($link,$only_dirs)) {
					$to_unset[] = $k;
				}
			}
		}
		foreach ($to_unset as $p) {
			unset($arResult[$p]);
		}
	}
}
