...
if ($mode == 'add_exel')
{
    if (isset($_POST['add_exel']))
    {
        ...
        else
        {
            ...
            $uploaddir = $_SERVER['DOCUMENT_ROOT'].'/tmp/'; /* путь куда сохраняем загруженный файл */
            $uploadfile = $uploaddir . basename($_FILES['blank-recipient']['name']);


            if (move_uploaded_file($_FILES['blank-recipient']['tmp_name'],$uploadfile)) {

                $res = parse_excel_file($uploadfile);   /* массив из exel файла  */
                foreach($res as $key=>$item){
                    if(empty($res[$key][0] || $res[$key][0])){
                        unset($res[$key]);
                    }
                }

                if(empty($res[0][0])){  /* если передан файл с произвольным расположением таблицы внутри */
                   /* удалить пустые элементы в подмассиве */
               ...
               if(!empty($res)){
                        ...
                        foreach($res as $key=>$value){
                            foreach($value as $k=>$val){
                               if(strlen($val)){
                                   if(mb_detect_encoding($val)=='UTF-8'){   /* смена кодировки строк */
                                       $text = utf8win1251($val);
                                       $res[$key][$k] = strip_tags(trim($text));
                                   }
                               }else{
                                   $res[$key][$k] ='Добавьте значение в поле!';
                               }
                        ...
                        foreach($res as $key=>$value){
                            if($key==0){
                                foreach($value as $k=>$v){
                                    if($v == 'COMPANY_SENDER' || $v == 'NAME_SENDER'
                                        || $v == 'PHONE_SENDER' || $v == 'CITY_SENDER'
                                        || $v=='INDEX_SENDER' || $v == 'ADRESS_SENDER'){
                                        $new_company[] = ['col'=>$v];
                                    }
                            ...
                            if($count>0){
                                for($i=1;$i<=$count;$i++){
                                    $arrResCompany[$i] = $new_company;
                                }
                                foreach($arrResCompany as $key=>$value){
                                    foreach($value as $k=>$v){
                                        $arrResCompany[$key][$k]['value'] = $res[$key][$k];  /* массив для записи в бд */
                                    }
                                }
                                ...
                                if($resadd){
                                    $_SESSION['MESSAGE'][] = "Получатели успешно добавлены";
                                    LocalRedirect($arParams['TYPE_LINK']."index.php");
                                }
                                else
                                {
