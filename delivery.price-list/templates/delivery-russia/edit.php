...
foreach ($arResult["HTML"]["CITIES"] as $key => $value) {
	...
	for ($j=0;$j<$arResult['size_types'];$j++) {
		...
		echo '<td><input type="text" size="3" value="'.$arResult["HTML"]["ORDERS"][$key][$name_type][$arResult["HTML"]["MIN"]].'" name="result['.$key.']['.$name_type.']['.$arResult["HTML"]["MIN"].']"></td>
		<td><input type="text" size="3" value="'.$arResult["HTML"]["ORDERS"][$key][$name_type][$arResult["HTML"]["MAX"]].'" name="result['.$key.']['.$name_type.']['.$arResult["HTML"]["MAX"].']"></td>';
		foreach ($arResult["HTML"]["DOCS"] as $key2 => $value) echo '<td><input type="text" size="3" value="'.$arResult["HTML"]["ORDERS"][$key][$name_type][$key2].'" name="result['.$key.']['.$name_type.']['.$key2.']"></td>';
		echo '<td><input type="text" size="3" value="'.$arResult["HTML"]["ORDERS"][$key][$name_type][$arResult["HTML"]["START_INDEX"]].'" name="result['.$key.']['.$name_type.']['.$arResult["HTML"]["START_INDEX"].']"></td>';
		foreach ($arResult["HTML"]["WEIGHT"] as $key2 => $value) echo '<td><input type="text" size="3" value="'.$arResult["HTML"]["ORDERS"][$key][$name_type][$key2].'" name="result['.$key.']['.$name_type.']['.$key2.']"></td>';
	}
	echo '</tr>';
}
...
<input type="submit" value=" " name="save" class="save">
</form>