...
if ($arResult['MODE'] != 'close')
{
	...
	if (($arResult['MODE'] == 'prints') || ($arResult['MODE'] == 'prints_mini'))
	{
		if (!$arResult['ADMIN_AGENT'])
		{
			$arResult['CURRENT_CLIENT'] = $agent_id;
        ...
        $arResult['LOGO_PRINT'] = CFile::GetPath($idlogoprint);
        $arResult['ADRESS_PRINT'] = GetSettingValue(718, false, $arResult['UK']);
		$arResult['INVOICES'] = false;
		$title = array();
		if (strlen(trim($_GET['id'])))
		{
			$arIds = explode(',',trim($_GET['id']));
			if (count($arIds) > 0)
			{
				$filter = array("IBLOCK_ID" => 83, "ID" => $arIds, "PROPERTY_CREATOR" => $arResult["CURRENT_CLIENT"]);
				$res = CIBlockElement::GetList(
					array("id" => "asc"), 
					$filter, 
					false, 
				...
				$arResult['TITLE'] = implode(', ', $title);
				$APPLICATION->SetTitle(implode(', ', $title));
				if ($arResult['MODE'] == 'prints_mini')
				{
					$arInvMini = array();
					...
					$arResult['INVOICES_MINI'] = array_chunk($arInvMini,3, true);
				}
			}
			else
			{
					...
					/*	$sitysender = explode(", ", $_POST['CITY_SENDER']);
						...
						$idlogoprint = GetSettingValue(716, false, $arResult['UK']);
							...
							//	$arPDF[ADRESS_PRINT] = $arResult['ADRESS_PRINT'];
								...
								$arPDF[REQUEST][TYPE_DELIVERY] = $_POST['TYPE_DELIVERY'];
								$arPDF[REQUEST][TYPE_PAYS] = $_POST['TYPE_PAYS'];
								...
								//$arPDF[REQUEST][ФамилияОтправителя] = 'Федоровский Александр';
								/*$arPDF[REQUEST][Габариты][Габарит_1][Габарит];
								$arPDF[REQUEST][Габариты][Габарит_1][КоличествоМест];
								$arPDF[REQUEST][Габариты][Габарит_1][ВесОтправления];
								$arPDF[REQUEST][Габариты][Габарит_1][sizes];
								$arPDF[REQUEST][Габариты][Габарит_2][Габарит];
								$arPDF[REQUEST][Габариты][Габарит_2][КоличествоМест];
								$arPDF[REQUEST][Габариты][Габарит_2][ВесОтправления];
								$arPDF[REQUEST][Габариты][Габарит_2][sizes];
								$arPDF[REQUEST][Габариты][Габарит_3][КоличествоМест];
								$arPDF[REQUEST][Габариты][Габарит_3][ВесОтправления];
								$arPDF[REQUEST][Габариты][Габарит_3][sizes];
								$arPDF[REQUEST][Габариты][Габарит_4][Габарит];
								$arPDF[REQUEST][Габариты][Габарит_4][КоличествоМест];
								$arPDF[REQUEST][Габариты][Габарит_4][ВесОтправления];
								$arPDF[REQUEST][Габариты][Габарит_4][sizes];
								$arPDF[REQUEST][Габариты][Габарит_5][Габарит];
								$arPDF[REQUEST][Габариты][Габарит_5][КоличествоМест];
								$arPDF[REQUEST][Габариты][Габарит_5][ВесОтправления]
								$arPDF[REQUEST][Габариты][Габарит_5][sizes];*/
						...
						{
							$event = new CEvent;
							$event->SendImmediate("NEWPARTNER_LK", "S5", array("DATE" => date('d.m.Y H:i'),"NUMBER" => $number_nakl), "N", 245);
						}
        ...
        if ($arResult['CURRENT_CLIENT'] > 0)
        {
			...
			if (isset($_POST['upload']))
			{
				...
				else
				{
					...
					if (intval($_POST['fileid']) > 0)
					{
						$arResult["FILE_ID"] = intval($_POST['fileid']);
					}
					else
					{
							...
							{	
								...
								if ($arResult["FILE_ID"])
								{
									...
									if ($arResult["CONTENT_TYPE"] == 'text/xml')
									{
										...
										$xml = simplexml_load_file($global_file);
										$arXml = xml2array($xml);
											...
											*/
											...
											foreach ($arXmlWin['InvoiceNew'] as $index => $inv)
											{
												...
												*/
												...
												if ((is_array($inv['Goods'])) && (count($inv['Goods']) > 0))
												{
													foreach ($inv['Goods'] as $good)
													{
														$arGoods[] = array(
															...
															'Amount' => intval($good['Amount']),
															'Price' => floatval(str_replace(',','.',$good['Price'])),
															'Sum' => floatval(str_replace(',','.',$good['Sum'])),
															'SumNDS' => floatval(str_replace(',','.',$good['SumNDS'])),
															'PersentNDS' => intval($good['PersentNDS'])
														);
													}
												}
												else
												...
												if (floatval(str_replace(',','.',$inv['Weight'])) > 0)
												{
													$resecho_weight = floatval(str_replace(',','.',$inv['Weight']));
												}
												elseif (floatval(str_replace(',','.',$inv['WBWeight'])) > 0)
												{
													$resecho_weight = floatval(str_replace(',','.',$inv['WBWeight']));
												}
												else
												{
												...
												if (intval($inv['Places']) >= 1)
												{
													$resecho_places = intval($inv['Places']);
												}
												elseif (intval($inv['WBPlaceCount']) >= 1)
												{
													$resecho_places = intval($inv['WBPlaceCount']);
												}
												else
												{
												...
												*/
												...
												$resecho = array(
													...
													564 => $Payment,
													565 => floatval(str_replace(',','.',$inv['Cost'])),
													733 => floatval(str_replace(',','.',$inv['CodCost'])),
													566 => floatval(str_replace(',','.',$inv['DeclaredCost'])),
													567 => $resecho_places,
													568 => $resecho_weight,
													569 => '',
												...
												if ((is_array($inv['Description'])) && (count($inv['Description']) > 0))
												{
													...
													{
														$sizesDescr = array(
															floatval(str_replace(',','.',$descr['Length'])),
															floatval(str_replace(',','.',$descr['Height'])),
															floatval(str_replace(',','.',$descr['Width']))
														);
														$arDescr[] = array(
															'name' => iconv('windows-1251','utf-8',$descr['Name']),
															'place' => intval($descr['Places']),
															'weight' => floatval(str_replace(',','.',$descr['Weight'])),
												...
												{
													$sizes = array(
														floatval(str_replace(',','.',$inv['Length'])),
														floatval(str_replace(',','.',$inv['Height'])),
														floatval(str_replace(',','.',$inv['Width']))
													);
