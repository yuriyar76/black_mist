...
$arResult['agent_array'] = GetCurrentAgent($USER->GetID());

if ($arResult['agent_array']['type'] == 51) {
	$arResult["ShowOnlyMyProducts"] = false;
	$arResult["CURRNET_SHOP"] = $_SESSION["CURRNET_SHOP"];
	$shop_info = GetAgentInfo($arResult["CURRNET_SHOP"]);
	$arResult["CURRNET_SHOP_NAME"] = $shop_info["NAME"];	
}
else {
	$arResult["ShowOnlyMyProducts"] = true;
	$arResult["CURRNET_SHOP"] = $arResult['agent_array']['id'];
	$arResult["CURRNET_SHOP_NAME"] =$arResult['agent_array']['name'];
					...
					foreach($_POST[$FIELD_ID] as $key=>$value)
					{
						if(is_array($value))
						{
							if(strlen($value["VALUE"])) {
								$f = str_replace(',','.',$value["VALUE"]);
								$arProps[$arField["ID"]][$key] = doubleval($f);
							}
						}
						else
						{
...
if($arResult["ELEMENT_ID"])
	$APPLICATION->SetTitle($arResult["ELEMENT_FIELDS"]["NAME"].": ".GetMessage("CC_EDIT"));
else
	$APPLICATION->SetTitle(GetMessage("CC_CREATE"));
