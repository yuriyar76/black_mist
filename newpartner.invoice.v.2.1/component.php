                    ...
                    if (strlen(trim($header[0])))
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (intval($arResult['AGENT']['PROPERTY_BY_AGENT_VALUE']) > 0)
                                {
                                    $db_props = CIBlockElement::GetProperty(40, intval($arResult['AGENT']['PROPERTY_BY_AGENT_VALUE']), array("sort" => "asc"), Array("CODE"=>"EMAIL"));
                                    if($ar_props = $db_props->Fetch())
                                    {
                                        if(strlen(trim($ar_props["VALUE"])))
                                        {
...
if ($arResult['MODE'] != 'close')
{
			...
			else
			{
				...
				if (count($_POST['ids']) > 0)
				{
					...
					*/
					...
					{
						$arResult["ERRORS"][] = 'Нет подключения к 1с<br>Загрузите накладные в 1с посредством файла манифеста.';
					}
	...
	if (($arResult['MODE'] == 'edit') || ($arResult['MODE'] == 'editNew'))
	{
		/*
		if (!$arResult['ADMIN_AGENT'])
		{
		...
		*/
		...
		if ((isset($_POST['save'])) || (isset($_POST['save-print'])) || (isset($_POST['save_ctrl'])))
		{
			...
			{
				AddToLogs('InvEditPostValues',$_POST);
				$_SESSION[$_POST["key_session"]] = $_POST["rand"];
	...
	if (($arResult['MODE'] == 'add') || ($arResult['MODE'] == 'addNew'))
	{
		if (!$arResult['ADMIN_AGENT'])
		{
			$arResult['CURRENT_CLIENT'] = $agent_id;
		...
		if ((isset($_POST['add'])) || (isset($_POST['add-print'])) || (isset($_POST['add_ctrl'])))
		{
			...
			{
				AddToLogs('InvAddPostValues',$_POST);
				$_SESSION[$_POST["key_session"]] = $_POST["rand"];
				if (!strlen($_POST['NAME_SENDER']))
				{
					$arResult["ERR_FIELDS"]["NAME_SENDER"] = 'has-error';
				...
				if (count($arResult["ERR_FIELDS"]) == 0)
				{
					...
					if (strlen(NewQuotes($_POST['NUMBER'])))
					{
						...
						$number_nakl = strlen(NewQuotes($_POST['NUMBER']));
					}
					else
					{
