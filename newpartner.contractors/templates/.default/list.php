...
foreach ($arResult['COMPANIES'] as $key => $value) {
    $COMPANY_SENDER = $value['company'];
    $NAME_SENDER = $value['name'];
    $PHONE_SENDER = $value['phone'];
    if (!empty($value['city_full'])) {
        $CITY_SENDER = $value['city_full'];
    } else {
        $CITY_SENDER = $value['city'];
    }
    $INDEX_SENDER = $value['index'];
    $ADRESS_SENDER = $value['adress'];
    $list[] = ['COMPANY_SENDER' => $COMPANY_SENDER,
...
$postfix = $arResult['AGENT']['ID'];
if(!$postfix){
    $postfix = $arResult['ADMIN_AGENT'];
}
$file_name = get_excel_file($list, " Справочник получателей.", $columns, $postfix);
...
<div class="row">
	...
	if ($arResult['OPEN']) 
	{
		if ($arResult['ADMIN_AGENT'])
		{
            ...
            <div class="col-md-4">
                <div class="btn-group">
                    <a href="<?=$arParams['TYPE_LINK'];?>index.php?mode=add" class="btn btn-warning"
                       id="add_customer">Новый <?=$arParams['TYPE_ONE_S'];?></a>
                    ...
                    <?php if($_SERVER['REQUEST_URI']==='/recipients/'):?>
                        <a href="<?=$arParams['TYPE_LINK'];?>index.php?mode=add_exel" class="btn btn-default"
                           data-toggle="tooltip" data-placement="bottom"
                             title="Загрузить список получателей в справочник из Exсel">
                        <span class="glyphicon glyphicon-cloud-upload" aria-hidden="true"></span>
                        </a>
                        <a href="<?=$arParams['TYPE_LINK'].$file_name;?>" class="btn btn-default"
                           data-toggle="tooltip" data-placement="bottom"
                           title="Выгрузить справочник получателей в Exсel">
            	...
            	<form action="" method="get" name="filterform" class="form-inline">
                    <div class="form-group">
                    	<select name="client" size="1" class="form-control selectpicker" id="client"
                                onChange="ChangeClient();" data-live-search="true" data-width="auto">
		...
		else
		{
            ...
            <?php if($_SERVER['REQUEST_URI']==='/recipients/'):?>
                        <a href="<?=$arParams['TYPE_LINK'];?>index.php?mode=add_exel" class="btn btn-default"
                           data-toggle="tooltip" data-placement="bottom"
                           title="Загрузить список получателей в справочник из exel">
                            <span class="glyphicon glyphicon-cloud-upload" aria-hidden="true"></span>
                        </a>
                        <a href="<?=$arParams['TYPE_LINK'].$file_name;?>" class="btn btn-default"
                           data-toggle="tooltip" data-placement="bottom"
                           title="Выгрузить справочник получателей в exel">
                            <span class="glyphicon glyphicon-cloud-download" aria-hidden="true"></span>
                        </a>
            <?php endif;?>
